...
function VitalCard({ vital }: { vital: VitalResult }) {
  ...
  return (
    <article className="group rounded-2xl border border-slate-200 bg-white p-4 shadow-[0_8px_30px_rgba(31,65,88,0.05)] transition-transform hover:-translate-y-0.5 sm:p-5">
      ...
      <p className="mt-4 text-sm font-semibold text-slate-500">{vital.label}</p>
      <p className="mt-1 flex items-baseline gap-1.5 text-2xl font-bold tracking-tight text-slate-950">
        {vital.value}
...
export function HealthChecker() {
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    setError("");
    setResult(evaluateVitals(vitals));
  }
    ...
    <main className="min-h-screen bg-[linear-gradient(135deg,#f7fbfd_0%,#ffffff_52%,#f5f8ee_100%)] text-slate-950">
      <header className="border-b border-sky-100/80 bg-white/85 backdrop-blur-xl">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-4 sm:px-8">
          <a href="#inicio" className="flex items-center gap-3 rounded-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-bolivar-blue">
            <span className="grid size-10 place-items-center rounded-xl bg-bolivar-blue text-white shadow-sm">
              <HeartPulse className="size-5" aria-hidden="true" />
            </span>
            <div>
              <p className="text-[0.7rem] font-bold uppercase tracking-[0.16em] text-bolivar-green">Clínica Bolívar</p>
              <p className="text-sm font-semibold text-bolivar-blue">Semáforo de Salud</p>
            </div>
          </a>
      ...
      {!result ? (
        <div id="inicio" className="mx-auto grid max-w-6xl gap-8 px-5 py-8 sm:px-8 lg:grid-cols-[0.78fr_1.22fr] lg:gap-12 lg:py-14">
          <section className="flex flex-col justify-center">
            <span className="mb-5 inline-flex w-fit items-center gap-2 rounded-full border border-sky-100 bg-white px-3 py-1.5 text-sm font-semibold text-bolivar-blue shadow-sm">
              <Activity className="size-4" aria-hidden="true" />
              Chequeo preventivo en 2 minutos
            </span>
            <h1 className="max-w-xl text-4xl font-bold leading-[1.08] tracking-[-0.04em] text-slate-950 sm:text-5xl lg:text-[3.45rem]">
              Tus signos vitales, explicados con claridad.
            </h1>
            ...
            <div className="mt-8 grid grid-cols-3 gap-3 text-center">
              {["Privado", "Rápido", "Preventivo"].map((item) => (
                <div key={item} className="rounded-2xl border border-white bg-white/75 px-2 py-4 text-sm font-semibold text-slate-600 shadow-[0_10px_30px_rgba(91,143,185,0.08)]">
                  {item}
                </div>
              ))}
            </div>
          ...
          <Card className="gap-0 overflow-hidden rounded-[1.75rem] border-sky-100 bg-white py-0 shadow-[0_24px_70px_rgba(31,65,88,0.12)]">
            <CardHeader className="gap-2 border-b border-sky-50 bg-sky-50/60 px-5 py-6 sm:px-7">
              <div className="flex items-center justify-between">
                <span className="text-sm font-bold uppercase tracking-[0.12em] text-bolivar-blue">Paso 1 de 2</span>
                <span className="text-sm font-medium text-slate-500">Signos vitales</span>
              </div>
                    ...
                    <label key={field.name} className="grid gap-2 text-sm font-semibold text-slate-700">
                      ...
                      <span className="relative">
                        <Input
                          ...
                          required
                          className="h-12 rounded-xl border-slate-200 bg-white pr-16 text-base shadow-none focus-visible:border-bolivar-blue focus-visible:ring-bolivar-blue/15"
                        />
        ...
        <section ref={resultRef} className="mx-auto max-w-6xl scroll-mt-5 px-5 py-7 sm:px-8 sm:py-10" aria-live="polite">
          <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
            ...
            <span className="text-sm font-bold uppercase tracking-[0.12em] text-bolivar-blue">Paso 2 de 2 · Resultado</span>
          </div>
          ...
          <div className="grid gap-6 lg:grid-cols-[0.92fr_1.08fr]">
            <Card className="gap-0 overflow-hidden rounded-[1.75rem] border-sky-100 bg-white py-0 shadow-[0_24px_70px_rgba(31,65,88,0.1)]">
              <CardContent className="flex h-full flex-col px-5 py-7 sm:px-8 sm:py-9">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-bold uppercase tracking-[0.13em] text-slate-400">Estado general</span>
                  <ShieldCheck className="size-5 text-bolivar-blue" aria-hidden="true" />
                </div>
                <GaugeChart status={result.status} score={result.score} />
                <div className="mt-auto text-center">
                  <h1 className="text-2xl font-bold tracking-tight text-slate-950 sm:text-3xl">{result.title}</h1>
            ...
            <div>
              <div className="mb-4 flex items-end justify-between">
                <div>
                  <p className="text-sm font-bold uppercase tracking-[0.12em] text-bolivar-blue">Tus mediciones</p>
                  <h2 className="mt-1 text-2xl font-bold tracking-tight text-slate-950">Desglose preventivo</h2>
                </div>
          ...
          <div className="mt-6 grid gap-6 lg:grid-cols-[1.08fr_0.92fr]">
            ...
            <section className="flex flex-col justify-between rounded-[1.5rem] bg-[#17384D] p-6 text-white shadow-[0_18px_50px_rgba(23,56,77,0.18)] sm:p-7">
              <div>
                <p className="text-sm font-bold uppercase tracking-[0.12em] text-sky-200">Tu bienestar continúa</p>
                <h2 className="mt-2 text-2xl font-bold tracking-tight">Vuelve a medir cuando lo necesites.</h2>
